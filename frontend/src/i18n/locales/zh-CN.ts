export default {
  ...
  common: {
    ...
    uploadSuccess: "上传成功！",
    uploadFailed: "上传失败！",
    uploadAllSuccess: "{count}个文件上传成功！",
    uploadPartialSuccess: "{success}个成功，{fail}个失败",
    uploadAllFailed: "{count}个文件上传失败",
    fileExists: "文件已存在",
    missingKbId: "知识库ID缺失",
  ...
  agent: {
    ...
    editor: {
      ...
      hints: {
        ...
        systemPromptBuiltinSuffix: "（留空使用系统默认值）",
        contextTemplateDesc: "定义检索到的内容传递给模型的格式",
        modelDesc: "选择智能体使用的大语言模型",
        temperatureDesc: "控制输出的随机性，0 最确定，1 最随机",
        maxCompletionTokensDesc: "模型生成响应的最大 Token 数",
        conversationSettingsDesc: "配置多轮对话和问题改写相关参数",
        multiTurnDesc: "启用后将保留之前的对话上下文",
        historyTurnsDesc: "保留最近几轮对话作为上下文",
        enableRewriteDesc: "多轮对话时自动改写用户问题，解决指代消解和省略补全",
        rewritePromptSystemDesc: "问题改写用的系统提示词（留空使用默认值）",
        rewritePromptUserDesc: "问题改写用的用户提示词模板（留空使用默认值）",
        allowedToolsDesc: "选择该智能体可以使用的工具",
        requiresKb: "（需要知识库配置）",
        maxIterationsDesc: "执行智能体任务时的最大推理步骤数",
        mcpServiceLabel: "MCP 服务",
        mcpServiceDesc: "选择智能体可以调用的 MCP 服务",
        selectMcpService: "选择 MCP 服务",
        selectMcpServiceDesc: "选择要启用的 MCP 服务",
        selectMcpServicePlaceholder: "选择 MCP 服务",
        all: "全部",
        selected: "指定",
        none: "禁用",
        kbRangeDesc: "选择智能体可访问的知识库范围",
        selectKbDesc: "选择要关联的知识库",
        faqPriorityStrategy: "FAQ 优先策略",
        faqPriorityTooltip: "如果知识库中包含 FAQ（问答对），可以启用此策略让 FAQ 回答优先于普通文档",
        enableFaqPriority: "启用 FAQ 优先",
        enableFaqPriorityDesc: "FAQ 回答优先于普通文档被引用，提高回答准确性",
        directAnswerThreshold: "直接回答阈值",
        directAnswerThresholdDesc: "当问题与 FAQ 的相似度超过此值时，直接使用 FAQ 答案",
        faqScoreBoost: "FAQ 分数加权",
        faqScoreBoostDesc: "将 FAQ 结果的相关性分数乘以此系数，提升排序",
        webSearchDesc: "启用后智能体可以从互联网搜索信息",
        webSearchMaxResultsDesc: "每次搜索返回的最大结果数",
        retrievalStrategyDesc: "配置知识库检索和排序参数",
        queryExpansionDesc: "自动扩展查询词以提高召回率",
        embeddingTopKDesc: "向量检索返回的最大结果数",
        keywordThresholdDesc: "关键词检索的最小相关性分数",
        vectorThresholdDesc: "向量检索的最小相似度分数",
        rerankTopKDesc: "重排序后保留的最大结果数",
        rerankThresholdDesc: "重排序的最小相关性分数",
        fallbackStrategyDesc: "当知识库中找不到相关内容时的处理方式",
        fixedReply: "固定回复",
        modelGenerated: "模型生成",
        fallbackResponseDesc: "无法回答时返回的固定文本",
        fallbackPromptDesc: "当知识库中找不到答案时，引导模型生成回答的提示词",
        builtinAgentNotice: "这是内置智能体，名称和描述不可修改，但可以调整配置参数",
        availableVariables: "可用变量：",
        clickToInsert: "（点击插入）",
        clickToInsertHint: "（点击插入或输入 {{ 呼出列表）",
      },
    },
    selector: {
      title: "选择智能体",
    ...
    tools: {
      ...
      listKnowledgeChunks: "查看文档分块",
      listKnowledgeChunksDesc: "获取文档完整分块内容",
      queryKnowledgeGraph: "查询知识图谱",
      queryKnowledgeGraphDesc: "从知识图谱中查询关系",
      getDocumentInfo: "获取文档信息",
      getDocumentInfoDesc: "查看文档元数据",
      databaseQuery: "查询数据库",
      databaseQueryDesc: "查询数据库中的信息",
    },
  },
  file: {
    upload: "上传文件",
  ...
  modelSettings: {
    ...
    toasts: {
      ...
      loadFailed: "加载模型列表失败",
      cannotEditBuiltin: "内置模型不能编辑",
      cannotDeleteBuiltin: "内置模型不能删除",
    },
